        ...
        while (cur != null) {
            if (treeNodeMap.get(cur.left) == null) {
                ...
                if (treeNodeMap.get(mostRight.right) == null) {
                    ...
                    mostRight.right = 0;
                    printTreesRightBorderReversely(treeNodeMap.get(cur.left), stringBuilder);
                    cur = treeNodeMap.get(cur.right);
                }
            }
        }
        printTreesRightBorderReversely(root, stringBuilder);
        result = stringBuilder.toString();
        return result;
    }

    private static void printTreesRightBorderReversely(TreeNode treeNode, StringBuilder stringBuilder) {
        TreeNode head = reverseRight(treeNode);
        printHeadToMostRight(head, stringBuilder);
        reverseRight(head);
    }

    private static void printHeadToMostRight(TreeNode head, StringBuilder stringBuilder) {
        while (head != null) {
            stringBuilder.append(head.id + " ");
            head = treeNodeMap.get(head.right);
        }
    }

    private static TreeNode reverseRight(TreeNode head) {
        TreeNode result;
        TreeNode pre = new TreeNode(0);
        while (head != null) {
            TreeNode next = treeNodeMap.get(head.right);
            head.right = pre.id;
            pre = head;
            head = next;
        }
