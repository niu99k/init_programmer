...
public class Main {
    ...
    private static void disposeInputTop(Integer pop) {
        if (support.isEmpty()) {
            ...
            transferAllFromSupport2InputUtilNoNumberBigger(pop);
            support.push(pop);
        }
    }

    private static void transferAllFromSupport2InputUtilNoNumberBigger(int compareBase) {
        while (!support.isEmpty()) {
            if (support.peek() <= compareBase) {
                break;
            }
